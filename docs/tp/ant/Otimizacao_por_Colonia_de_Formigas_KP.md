...
### 5.1 Nomes usados no código/propriedades (alinhados com o documento)

Para facilitar a leitura do código e da configuração experimental:

- Número de formigas `m` → `mmas.numero.formigas`
- Número de ciclos → `mmas.numero.ciclos`
- `α` (peso feromona) → `mmas.peso.feromona`
- `β` (peso heurística) → `mmas.peso.heuristica`
- `ρ` (evaporação) → `mmas.taxa.evaporacao`
- Intensidade de depósito (`q`) → `mmas.intensidade.deposito`
- Ciclos sem melhoria (estagnação) → `mmas.ciclos.sem.melhoria`
- Semente aleatória → `mmas.semente`
- CSV de saída → `mmas.saida.csv`
